  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signin") {
        ...
        // If email confirmation is required or if session is null / user created
        setVerificationSent(true);
        toast.success("Registration successful! Please check your inbox for verification.");
      }
    } catch (err: any) {
      setBusy(false);
    ...
    <Card className="w-full max-w-md mx-auto shadow-md">
      ...
      <CardContent className="space-y-6">
        ...
        <Button
          ...
          disabled={busy}
          className="w-full h-11 font-medium flex items-center justify-center gap-2 hover:bg-muted/80"
        >
          {busy ? (
            <>
